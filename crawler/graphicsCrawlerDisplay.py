...
class Application:
    ...
    def __initGUI(self, win):
        ...
        ## Exit Button ##
        # self.exit_button = tkinter.Button(win,text='Quit',
        # command=self.exit)
        # self.exit_button.grid(row=0,
        # column=9)
    ...
    def setupAlphaButtonAndLabel(self, win):
        ...
        self.alpha_minus.grid(
            row=1,
            column=3,
            padx=10)

        self.alpha = self.sigmoid(self.al)
        self.alpha_label = tkinter.Label(win, text=f'Learning Rate: {self.alpha}')
    ...
    def setUpGammaButtonAndLabel(self, win):
        ...
        self.gamma_minus.grid(
            row=1,
            column=0,
            padx=10)

        self.gamma = self.sigmoid(self.ga)
        self.gamma_label = tkinter.Label(win, text=f'Discount: {self.gamma}')
    ...
    def setupEpsilonButtonAndLabel(self, win):
        ...
        self.epsilon_minus.grid(
            row=0,
            column=3)
        self.epsilon = self.sigmoid(self.ep)
        self.epsilon_label = tkinter.Label(win, text=f'Epsilon: {self.epsilon}')
        self.epsilon_label.grid(
            row=0,
            column=4)
    ...
    def setupSpeedButtonAndLabel(self, win):
        self.speed_minus = tkinter.Button(
            win,
            text="-",
            command=(lambda: self.incrementSpeed(.5)))
        self.speed_minus.grid(
            row=0,
            column=0)

        self.speed_label = tkinter.Label(win, text=f'Step Delay: {self.tickTime}')
        ...
        self.speed_plus = tkinter.Button(
            win,
            text="+",
            command=(lambda: self.incrementSpeed(2)))
        self.speed_plus.grid(
            row=0,
            column=2
        )
    ...
    def __init__(self, win):
        ...
        actionFn = lambda state: \
            self.robotEnvironment.get_list_action_direction_possible(state)
    ...
    def step(self):
        ...
        actions = self.robotEnvironment.get_list_action_direction_possible(state)
        if len(actions) == 0.0:
