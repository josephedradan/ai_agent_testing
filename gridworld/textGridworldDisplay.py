...
class TextGridworldDisplay:
    ...
    def displayQValues(self, agent, currentState = None, message = None):
        ...
        states = self.gridworld.getStates()
        for state in states:
            for action in self.gridworld.get_list_action_direction_possible(state):
    ...
    for y in range(grid.height):
        ...
        for x in range(grid.width):
            ...
            if policy != None and state in policy:
                action = policy[state]
            actions = gridWorld.get_list_action_direction_possible(state)
...
if __name__ == '__main__':
    ...
    stateCrossActions = [[(state, action) for action in grid.get_list_action_direction_possible(state)] for state in grid.getStates()]
